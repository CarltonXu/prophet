...
新版应用管理提供可视化画布，用于梳理业务中主机之间的依赖关系，使用步骤如下：

1. **资源面板**
   - 左侧资源面板支持检索未加入应用的主机，可一键将其加入画布并同步到后端。
   - 同时提供网络、存储、服务等模板节点，便于补充业务架构中尚未采集到的逻辑组件。

2. **画布与工具栏**
   - 顶部工具栏支持网格、横向、纵向布局快速排版，并提供缩放、视图复位能力。
   - 画布节点支持拖拽移动，连线即可新建主机关系；删除节点/连线会同步更新后台数据。

3. **属性面板**
   - 右侧属性面板会显示当前选中的节点或连线，支持修改关系类型、描述，或重新绑定主机。
   - 对于模板节点，可直接在此修改展示名称，以便描述业务环境。

> 提示：模板节点目前仅用于画布可视化，不会写入数据库，可配合真实主机节点标注业务拓扑。
